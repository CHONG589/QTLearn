...
public:
     ...
     * @exception DBException 当 SQL 执行失败时抛出
     */
     ...
     * @exception DBException 查询执行失败时抛出
     */
     ...
     * @exception DBException 预编译失败时抛出
     * @note 典型用法：
     * @code
     ...
     * @exception DBException 绑定或执行失败时抛出
     */
     ...
     * @param[in] sql  含占位符的 SQL 模板
     * @param[in] args 参数列表
     * @exception DBException 任一阶段失败时抛出
     */
     ...
     * @exception DBException 事务开启失败时抛出
     */
    void begin();
...
public:
     ...
     * @exception DBException 未初始化或获取超时时抛出
     */
    DBConn acquire();
...
private:
     ...
     * @exception DBException 打开连接失败时抛出
     */
...
public:
    /**
     * @brief 构造时从池中获取一个连接
     * @exception DBException 连接池获取超时时抛出
     */
    ScopedConn() {
    ...
    ~DBTransaction() {
        if (m_active) {
            try {
                ...
                LOG_WARN(LOG_ROOT()) << "DBTransaction rollback failed: " << e.what();
            }
        }
    }
