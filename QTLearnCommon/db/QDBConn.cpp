...
QSqlDatabase DBPool::createConnection() {
    ...
    try {
        db.setUserName(QString::fromStdString(Crypto::decrypt(g_db_user->getValue().toStdString())));
        db.setPassword(QString::fromStdString(Crypto::decrypt(g_db_pwd->getValue().toStdString())));
